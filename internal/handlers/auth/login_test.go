...
func TestLogin(t *testing.T) {
	...
	tests := []struct {
		...
		mockSetup               func(*mocks.MockloginService)
		wantStatusCode          int
	...
	}{
		{
			...
			mockSetup: func(m *mocks.MockloginService) {
				m.EXPECT().
					Login(gomock.Any(), gomock.Eq("user1"), gomock.Eq("pass123")).
		...
		},
		{
			...
			requestBody: map[string]interface{}{"login": "user1", "password": "wrongpass"},
			mockSetup: func(m *mocks.MockloginService) {
				m.EXPECT().
					Login(gomock.Any(), gomock.Eq("user1"), gomock.Eq("wrongpass")).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockLoginService := mocks.NewMockloginService(ctrl)
			w := httptest.NewRecorder()
			ctx, _ := gin.CreateTestContext(w)
			ctx.Request = httptest.NewRequest(http.MethodPost, "/login", body)
			ctx.Request.Header.Set("Content-Type", "application/json")
			tt.mockSetup(mockLoginService)
			NewLoginHandler(mockLoginService)(ctx)
